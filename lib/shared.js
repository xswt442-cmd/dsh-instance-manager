...
// The loopback predicates and the request guard are not written here. They are
...
// error vocabulary. Edit the dock fragment, then run `npm run guard:sync`.
...
// These three values are the closed set the dock protocol accepts. Panels
// other than this one position themselves off `--createhelper-utility-dock-left`
// and `html[data-createhelper-utility-dock-placement]`, both driven by the
// selected value, so renaming or adding a fourth changes a published contract

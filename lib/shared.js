 ...
 * — an empty parse must not read as a pass. Kept separate from `createGuard` so
 * the API route and the guard apply the exact same criterion (no drift between
 * the two gates).
 */
...
export const createGuard = ({ currentPort, respond, allowRemoteHost }) => (req, res) => {
  ...
  const host = req.headers.host || ''
  // The socket peer is the only network signal a client cannot forge; the
  // Host header is attacker-controlled. A missing address means the caller is
  // unidentified, so fail closed — every real HTTP connection carries one.
  // `undefined` (no socket) and `''` (address not filled in) are the same
  // answer to "who is this peer?" — unidentified. Only a real connection
  ...
  if (peerAddress == null || String(peerAddress).trim() === '') {
    respond(res, 403, { ok: false, code: 'unknown_peer', error: '无法确定对端地址' })
    return false
  }
  const peerLoopback = isLoopbackAddress(peerAddress)
  const parsedHost = host ? hostHostname(host) : ''
  // An empty parse is off-loopback: an unbracketed IPv6 literal (`::1:3080`) is
  // not a valid Host per RFC 7230, and letting it read as a pass would skip the
  // allowlist. An ABSENT Host still counts as loopback — see requestNeedsBearer.
  const hostLoopback = host ? (parsedHost !== '' && isLoopbackName(parsedHost)) : true
  // Remote = off-loopback peer OR off-loopback Host. Either makes the request
  // untrusted; it may proceed only in fleet mode (allowRemoteHost), where the
  // handler verifies the bearer itself. The Host criterion stays so a local
  // request that spoofs an off-loopback Host is still challenged — no
  // relaxation versus the old Host-only rule.
  const remote = !peerLoopback || !hostLoopback
  if (remote && !(typeof allowRemoteHost === 'function' && allowRemoteHost())) {
    respond(res, 403, { ok: false, code: 'bad_host', error: '远程请求需要 fleet 模式' })
    return false
  }
  ...
  if (origin) {
    ...
    try {
      ...
      // portOf normalises default ports, so `http://127.0.0.1` (no port)
      // matches a server bound to 80 — WHATWG strips the default port, which
      // previously caused a false mismatch and a 403 on a legitimate request.
      same = isLoopbackName(hostHostname(o.hostname)) && portOf(o) === String(currentPort() || '')
    } catch (e) { same = false }
    if (!same) {

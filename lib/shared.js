 ...
 * A missing Host is treated as loopback here so a bare local request is not
 * rejected on host grounds; the peer address is what actually decides trust.
 * Kept separate from `createGuard` so the API route and the guard apply the
 * exact same criterion (no drift between the two gates).
 */
export const requestNeedsBearer = (req) => {
  const peerLoopback = isLoopbackAddress(req.socket && req.socket.remoteAddress)
  const host = req.headers.host || ''
  const hostLoopback = host ? isLoopbackName(hostHostname(host)) : true

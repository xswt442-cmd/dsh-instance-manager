...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PanelBody() {
      ...
      // Remote rows share port numbers with OTHER machines: their logs and
      // sessions are routed THROUGH the peer link (F3) by passing the peer id
      // the host forwards on; rows without one stay local.
      const remoteOf = (port) => {
        const it = data && Array.isArray(data.items) && data.items.find((i) => i.port === port && i.remote)
        return it ? it.source : undefined
      }
      const loadLogs = async (port, stream) => {
        try {
          const r = await api({ action: 'logs', port: port, stream: stream, peer: remoteOf(port) })
      ...
      const loadSessions = async (port) => {
        try {
          const r = await api({ action: 'sessions', port: port, peer: remoteOf(port) })
          if (r) setSessData(r)
        } catch (err) { }
      }
      const refresh = async () => {

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Small page-local registry shared with dsh-treekeeper. Each plugin owns
    // its panel; the dock only aggregates explicit launchers and follows the
    // live sidebar edge. Keeping the protocol on window lets either plugin
    // load first and gives future createhelper utilities the same entry point.
    ...
    const STRINGS = {
      zh: {
        ...
        sessMore: (n) => '… 还有 ' + n + ' 个',
        sessSub: '子代理',
        remoteHint: '远程实例的日志与会话查看将在后续版本支持',
      ...
      en: {
        ...
        foot: 'auto-refresh 4s · click a row for details',
        ...
        sessMore: (n) => '… ' + n + ' more',
        sessSub: 'subagent',
        remoteHint: 'Remote logs & sessions land in a later release',
    ...
    function Drawer(props) {
      ...
      return h('div', { className: 'dshim-drawer' },
        ...
        it.remote ? h('div', { key: 'rh' }, t.remoteHint) : null,
    ...
    function Row(props) {
      ...
      return h('div', {
        ...
        h('button', {
          ...
          disabled: !!busy[it.port] || it.managed === false || !!it.remote,
    ...
    function PanelBody() {
      ...
      // Remote rows share port numbers with OTHER machines: loading their
      // logs/sessions through the local forwarder would hit whatever listens
      // on that port HERE. Gated until F3 wires the link transport.
      const isRemotePort = (port) => !!(data && Array.isArray(data.items) &&
        data.items.some((i) => i.port === port && i.remote))
      const loadLogs = async (port, stream) => {
        if (isRemotePort(port)) return
        try {
          const r = await api({ action: 'logs', port: port, stream: stream })
          if (r && r.ok) setLogData(r)
        } catch (err) { }
      }
      // Session summary for the drawer: failures are kept (not swallowed) so
      // the drawer can say "legacy/unreachable" instead of spinning.
      const loadSessions = async (port) => {
        if (isRemotePort(port)) return
        try {
          const r = await api({ action: 'sessions', port: port })
          if (r) setSessData(r)
        } catch (err) { }
      }
      const refresh = async () => {
              ...
              [].concat.apply([], items.map((it) => [
                ...
                expanded === it.port
                  ? h(Drawer, {
                    ...
                    sess: sessData && sessData.port === it.port ? sessData : null,

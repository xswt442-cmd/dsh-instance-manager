    ...
    function PanelBody() {
      ...
      const stop = async (item) => {
        ...
        if (item.current) {
          ...
          // window.close() is honored by most browsers only for windows the
          // script opened itself, so the visible fallback message is the real
          // UX and the close call is a courtesy attempt.
      ...
      const stopAll = async () => {
        ...
        // land. Fire-and-forget, give the peers a moment, then try to close
        // this tab (best-effort) and settle on the final fallback message.

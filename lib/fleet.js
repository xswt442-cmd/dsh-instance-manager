...
// socket is dual-role: hello announces identity, `query` frames are answered
// locally, `query-result` frames settle our own outbound queries.
//
...
//     (fail closed: no token, no channel).
...
const WS_OPEN = 1
// A peer that accepts TCP but never finishes the 101 upgrade must not wedge
// the link in CONNECTING forever.
const HANDSHAKE_TIMEOUT_MS = 10 * 1000

// Bounds every peer may drive at us. Both are about cost, not correctness:
// answering a `fleet` query is a full 50-port sweep on this machine, so an
// authenticated peer can otherwise make every instance list all day.
//
// MAX_FRAME_BYTES goes to `ws` as maxPayload and covers any frame, so even a
// rewidened window cannot hand a 100 MiB text frame to JSON.parse.
// QUERY_LIMIT counts *query* frames only — hello and the answers to our OWN
// outbound queries are not work we performed for free.
//
// Exported as a set so the unit test can drive the boundary without waiting
// out a real window or guessing what changed.
export const MAX_FRAME_BYTES = 64 * 1024
export const QUERY_WINDOW_MS = 5000
...
export const parsePeers = (raw, cap = 16) => {
  ...
  for (const part of String(raw || '').split(',')) {
    ...
    if (!m) continue
    const id = m[1]
    // A peer authenticates with the fleet bearer token, never with credentials
    // embedded in the URL. Userinfo is rejected rather than carried: it would
    // travel in the WebSocket URL and surface in error details. Strip the scheme
    // first so the check sees userinfo in `scheme://user:pass@host` too.
    if (/^[^/]*@/.test(m[2].replace(/^https?:\/\//, ''))) continue
...
class LinkState {
  ...
  dial() {
    ...
    this.fleet.resolveToken().then((token) => {
      ...
      this.socket = socket
      // A peer that accepts TCP but never finishes the 101 upgrade would wedge
      // the link in CONNECTING forever: 'close' never fires, the backoff loop
      // never restarts, and the peer reads as permanently unreachable with no
      // trace. Bound the handshake; terminating makes 'close' fire, which
      // restores the retry loop.
...
/**
 * The per-instance fleet link hub (dual-role): inbound half answers peer
 * queries; outbound half dials configured peers.
...
export class FleetLinks {
  constructor(deps, peers) {
    ...
    this.server = new deps.WebSocketServer({ noServer: true, maxPayload: MAX_FRAME_BYTES })
    this.inbound = new Set()
    this.links = new Map() // peerId -> LinkState
    this.pending = new Map() // reqId -> { resolve, timer, socket }
    this.queryBudget = new WeakMap() // socket -> recent query timestamps
    this.reqIdCounter = 0
  ...
  handleSocketFrame(peerId, websocket, text) {
    ...
    if (frame.type === 'query-result' && frame.reqId) {
      ...
      if (!p) return
      // Only the socket OUR query went out on may settle it. reqIds are a
      // counter plus a timestamp (`q1-<base36 now>`), so with any socket
      // accepted an authenticated peer could answer somebody else's pending
      // query by guessing the id — the tie has to be the connection itself.
      if (p.socket !== websocket) return
    ...
    if (frame.type === 'query' && frame.reqId) {
      if (!this.allowQuery(websocket)) {
        // Answer rather than drop: the peer is waiting on a correlated reply,
        // and a silent frame leaves it held until its own timeout expires.
        this.sendTo(websocket, { type: 'query-result', reqId: frame.reqId, result: { ok: false, code: 'rate_limited' } })
        return
      }
  ...
  /** Token window of recent query frames per socket; false when it is spent. */
  allowQuery(websocket) {
    const now = Date.now()
    const recent = (this.queryBudget.get(websocket) || []).filter((at) => now - at < QUERY_WINDOW_MS)
    if (recent.length >= QUERY_LIMIT) {
      this.queryBudget.set(websocket, recent)
      return false
    }
    recent.push(now)
    this.queryBudget.set(websocket, recent)
  ...
  queryPeer(peerId, query, timeoutMs = 3500) {
    ...
    // Bound to the socket the query left on: only that socket's
    // `query-result` may settle it (see handleSocketFrame). A link that
    // reconnects before the answer arrives therefore times out instead of
    // accepting a frame from the replacement connection.
    ...
    return new Promise((resolve) => {
      ...
      this.pending.set(reqId, { resolve, timer, socket })
    })
  }

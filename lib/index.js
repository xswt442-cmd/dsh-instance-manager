    ...
    resolveWsLib().then((WS) => {
      ...
      fleet = new FleetLinks({
        ...
        answerQuery: async (q) => {
          ...
          // Logs delegate to the same helper the local route uses, so the
          // port validation there is the one a peer is held to.
      ...
      handler: async (req, res) => {
        try {
          ...
          if (action === 'logs') {
            ...
            const r = await logsFor(u.searchParams.get('port'), stream, peer)

...
export default {
  ...
  apply(ctx) {
    ...
    const sessionsFor = async (port, peer) => {
      if (peer) {
        const r = await fleetQuery(peer, { kind: 'sessions', port: port || undefined }, 5000)
    ...
    const logsFor = async (port, stream, peer) => {
      if (peer) {
        const r = await fleetQuery(peer, { kind: 'logs', port, stream }, 5000)
        return r && r.ok ? Object.assign({}, r, { peer })
          : { ok: false, code: 'logs_unavailable', error: 'peer 未提供日志（离线或版本过旧）' }
      }
      return Object.assign({ ok: true, port, stream }, readLogsFor(port, stream))
    }
    ...
    resolveWsLib().then((WS) => {
      ...
      fleet = new FleetLinks({
        ...
        answerQuery: async (q) => {
          ...
          if (q.kind === 'sessions') return describeSessions()
          if (q.kind === 'logs') {
            const stream = q.stream === 'err' ? 'err' : 'out'
            return Object.assign({ ok: true, port: q.port, stream }, readLogsFor(q.port, stream))
          }
    ...
    const fleetTick = async () => {
      try {
        ...
        // machines; their flapping never toasts as instance up/down.
      ...
      handler: async (req, res) => {
        try {
          ...
          if (action === 'logs') {
            ...
            const peer = u.searchParams.get('peer') || undefined
            sendJson(res, 200, await logsFor(port, stream, peer))
            return
          }
          if (action === 'sessions') {
            ...
            const peer = u.searchParams.get('peer') || undefined
            sendJson(res, 200, await sessionsFor(qp, peer))
            return
          }
          if (action === 'start') {

...
export default {
  ...
  apply(ctx) {
    ...
    const launchOnce = async ({ browserHandoff = false, port: requestedPort } = {}) => {
    ...
    const startWithRetry = async (options) => {
      let r = await launchOnce(options)
      // The retry exists for losing the scan/bind race for an AUTO-picked port.
      // When the caller named a port, retrying would start an instance somewhere
      // the caller did not ask for, so a failure there is reported as-is.
      const autoPort = !options || options.port === undefined
      if (autoPort && !r.ok && r.code === 'start_failed') {
        // One retry on the next free port — the usual cause is having lost
        // the scan/bind race for the first pick.
        const second = await launchOnce(options)

...
export default {
  ...
  inject: ['webServer'],
  // `config` is the profile entry's configuration (cordis hands it to apply as
  // the second argument). From 0.1.7-rc.1 that is where a plugin's settings
  // live; on older lines it is usually absent and the settings service below
  // serves instead.
  apply(ctx, config) {
    ...
    // Section getters. Two sources, newest first:
    //
    // 1. the plugin's own config — from 0.1.7-rc.1 the profile ENTRY owns a
    //    plugin's configuration (cordis hands it to `apply` as the second
    //    argument, and editing it restarts this fiber, which is why the peer
    //    list needs no watch on that line: the values are re-read on apply).
    // 2. `settings.register(namespace, schema, …)` — the pre-0.1.7 way, gone
    //    from 0.1.7-rc.1 (the service became SettingsForms with
    //    describe/update/replace/mutate and no register at all).
    //
    // `startupGet` is the applies:'restart' consumer: read per use, but the
    // owner never watches, so the value only changes across restarts — same
    // semantics as the old load-time PORT_RANGE const.
    const entryConfig = config && typeof config === 'object' ? config : {}
    const configServes = [entryConfig.live, entryConfig.startup]
      .some((section) => section && typeof section === 'object' && Object.keys(section).length > 0)
    let liveGet = () => resolveLiveSection(entryConfig.live, process.env)
    let startupGet = () => resolveStartupSection(entryConfig.startup, process.env)
    // Peers start from the effective section, whichever source produced it:
    // on 0.1.7 that is the entry config, on older lines the env base until the
    // settings watch below replaces it.
    peers = parsePeers(liveGet()[PEERS_FIELD])
    currentPeersRaw = liveGet()[PEERS_FIELD] || ''
    if (typeof ctx.inject === 'function') {
      ctx.inject(['settings'], (settingsCtx) => {
        ...
        // Where `register` no longer exists the entry config above already
        // serves, and registering anyway would only log a false failure.
        if (!settings || typeof settings.register !== 'function') return
        if (configServes) {
          warn('dsh-instance-manager: 该版本同时提供了插件配置与旧的设置服务，已以插件配置为准')
        }
    ...
    // These logs are written on rare failure paths and rotated by nothing
    // else, so cap them: past the ceiling keep the most recent half and
    // continue. (The launcher-owned server-<port>.out.log is deliberately not
    // capped here — the child process holds that fd, so a rename would fork
    // the file behind the writer's back.)
    const appendCapped = (file, line, maxBytes = 1024 * 1024) => {
      try {
        let size = 0
        ...
        if (size > maxBytes) {
          try {
            ...
            fs.writeFileSync(file, prev.slice(Math.floor(prev.length / 2)))
          } catch (e) { return }
        }
        fs.appendFileSync(file, line)
      } catch (e) { /* diagnostics must never fail what they describe */ }
    }
    const crashLog = (kind, detail) => {
      try {
        const dir = path.join(dshHome(), 'launcher', 'logs')
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
        appendCapped(path.join(dir, 'dshim-crash.log'),
    ...
    const noteSelfExit = (trigger) => {
      try {
        const dir = path.join(dshHome(), 'launcher', 'logs')
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
        appendCapped(path.join(dir, 'dshim-selfexit.log'),
    ...
    // The self-exit breadcrumb names the TRIGGER, which answers "was this a
    // crash or a request" but never "who asked". A mutation is rare, deliberate
    // and, for `stop` on this very port, the last thing that happens before
    // the process leaves, so record its provenance at the moment it is
    // accepted: the socket peer, the admission path that let it in, and the
    // browser/host metadata a page cannot avoid sending.
    //
    // That split is the whole point: a panel click arrives as a same-origin
    // POST carrying an Origin, a Referer and a browser User-Agent, while a
    // script or another process carries none of them. This file is a trail, not
    // a credential store: cookies and Authorization are never read.
    const noteRequest = (action, req, detail) => {
      try {
        const headers = req.headers || {}
        const field = (name) => String(headers[name] == null ? '' : headers[name]).replace(/[\r\n]+/g, ' ').slice(0, 200)
        const dir = path.join(dshHome(), 'launcher', 'logs')
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
        appendCapped(path.join(dir, 'dshim-requests.log'),
          '[' + new Date().toISOString() + '] action=' + action +
          ' instance=' + String(currentPort()) +
          ' target=' + (detail.port === undefined || detail.port === null || detail.port === '' ? '-' : String(detail.port)) +
          ' via=' + String(detail.via || 'unknown') +
          ' peer=' + String((req.socket && req.socket.remoteAddress) || '-') +
          ' host=' + field('host') +
          ' origin=' + field('origin') +
          ' referer=' + field('referer') +
          ' ua=' + field('user-agent') +
          ' result=' + String(detail.result || 'accepted') + '\n')
      } catch (e) { /* diagnostics must never fail the action they describe */ }
    }
    ...
    const listLocalInstances = async () => {
      ...
      return {
        items: orderInstances(results.filter(Boolean)),
    ...
    //
    // `orderInstances` is applied to the MERGED list, not to each half: the
    // current-instance-first rule only holds if the row can move across the
    // local/remote boundary, and concatenating two already-ordered halves
    // would leave the current row first only when it happens to be in the
    // first half.
    const listInstances = async () => {
      const local = await listLocalInstances()
      const peerFleets = await collectPeerFleets()
      return {
        items: orderInstances(local.items.concat(peerFleets.rows)),
    ...
    resolveWsLib().then((WS) => {
      ...
      // Without this line a throwing resolveWsLib leaves the fleet link
      // permanently offline with zero diagnostics.
      console.error('dsh-instance-manager: fleet mount failed:', String((e && e.message) || e))
    })
    ...
    // Every subscriber costs one full fleet listing on connect (a local port
    // scan plus peer queries) and one frame per tick, so the stream needs a
    // ceiling: an unbounded EventSource loop would turn one page into a local
    // scan amplifier.
    ...
    const broadcastFleet = (obj) => {
      ...
      for (const res of sseClients) {
        ...
        try {
          // write() === false means the socket buffer is full: drop the slowest
          // subscriber instead of queueing unboundedly. It can reconnect; the
          // tick it missed is recoverable from the baseline frame.
          if (res.write(frame) === false) sseClients.delete(res)
    ...
    const apiRoute = {
      ...
      handler: async (req, res) => {
        try {
          ...
          // Which admission path opened this request. Recorded with every
          // accepted mutation, because "a fleet peer asked" and "a page on this
          // loopback asked" are different answers to the same stop.
          let via = 'local-browser'
          ...
          if (action === 'start') {
            ...
            const started = await startWithRetry({ browserHandoff: true, port: startPort })
            noteRequest('start', req, { via, port: startPort, result: started && started.port ? 'spawned:' + started.port : 'not-spawned' })
          ...
          if (action === 'stop') {
            ...
            if (port === currentPort()) {
              noteRequest('stop', req, { via, port, result: 'self-exit' })
            ...
            noteRequest('stop', req, { via, port, result: r && r.ok ? 'forwarded' : 'stop_unconfirmed' })
            if (r && r.ok) {
          ...
          if (action === 'stop-all') {
            ...
            noteRequest('stop-all', req, { via, result: 'self-exit;forwarded=' + acked.length })
          ...
          if (action === 'stop-self') {
            ...
            noteRequest('stop-self', req, { via, result: 'self-exit' })
    ...
    const stopForAgent = async (rawPort) => {
      ...
      handler: async (req, res) => {
        try {
          ...
          if (sseClients.size >= MAX_SSE_CLIENTS) {
            ...
            res.end(JSON.stringify({ ok: false, code: 'too_many_streams', error: 'too many event streams; close one and retry' }))
            return
          }
          res.writeHead(200, {

...
export default {
  ...
  apply(ctx) {
    ...
    // fatal path to ONE shared breadcrumb log FIRST, then preserve the crash
    // semantics (exit 1). Instances share the file; pid+port disambiguate.
    // Never swallow: if this plugin is not the cause, the evidence still says
    // exactly who is.
    ...
    const launchOnce = async () => {
      ...
      // --report-on-uncaught-exception drops a full diagnostic report beside
      // the crash black-box, and strict rejections make the default explicit.
      const child = spawn(picked.node,
        ['--trace-exit', '--unhandled-rejections=strict', '--report-on-uncaught-exception', picked.bin, 'web', '--no-open', '--port', String(port)], {
        cwd: dshHome(),
        detached: true,

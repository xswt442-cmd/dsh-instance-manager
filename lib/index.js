...
//
// Security model (README "瀹夊叏妯″瀷"): every action runs through a guard that
// rejects browser-initiated cross-site traffic 鈥?Fetch Metadata
...
export default {
  ...
  apply(ctx) {
    ...
    // The host process itself is node 鈥?spawning children with its own
    ...
    // ---- request guards (see README "瀹夊叏妯″瀷") -------------------------
    ...
    const requirePost = (req, res, action) => {
      ...
      sendJson(res, 405, { ok: false, code: 'need_post', action, error: action + ' 闇€瑕?POST 璇锋眰' })
      return false
    }
    ...
    const scheduleSelfExit = () => {
      const fire = () => {
        ...
        if (exit) {
          ...
          // event loop draining 鈥?a lingering handle can hold the process
    ...
    const startInstance = () => {
      const node = findNode()
      const bin = dshBin()
      if (!node) return { ok: false, code: 'no_node', error: '鏈壘鍒?node 鍙墽琛屾枃浠? }
      if (!bin) {
        const detail = path.join(dshHome(), 'profiles', 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js')
        return { ok: false, code: 'no_dsh_bin', detail, error: '鏈壘鍒?dsh 鍚姩鍣細' + detail }
      }
    ...
    // junk files fail validation and are ignored 鈥?the sweep re-covers them.
    ...
    // answers action=self on the CURRENT route (a fresh entry promises 鈮?this
    ...
    const listInstances = async () => {
      ...
      const results = await Promise.all(ports.map(async (port) => {
        ...
        // Current and pre-rename self-report routes race concurrently 鈥?the
        ...
        // Every dsh web index page carries the injected boot manifest 鈥?a far
      ...
      handler: async (req, res) => {
        try {
          ...
          if (action === 'logs') {
            ...
            // The port only ever becomes part of a fixed filename pattern 鈥?            // no path-traversal surface.
            ...
            let result
            try {
              result = tailFile(path.join(dshHome(), 'launcher', 'logs', 'server-' + port + '.' + stream + '.log'))
            } catch (e) {
              result = { exists: false, truncated: false, lines: [] }
            }
            sendJson(res, 200, Object.assign({ ok: true, port, stream }, result))
            return
          }
          if (action === 'start') {
            ...
            if (!port) {
              sendJson(res, 200, { ok: false, code: 'no_free_port', error: '3080鈥?129 绔彛鍏ㄩ儴琚崰鐢? })
              return
            }
          ...
          if (action === 'stop') {
            ...
            sendJson(res, 200, { ok: false, code: 'stop_unconfirmed', error: '鐩爣瀹炰緥娌℃湁纭鍋滄锛堟湭鎸傝浇鏈鐞嗛潰鏉挎垨涓嶅彲杈撅級' })
            return
          }
          if (action === 'stop-all') {

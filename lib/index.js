      ...
      handler: async (req, res) => {
        try {
          ...
          if (action === 'start') {
            ...
            // Pick the first port with no listener (shared scanner) and spawn
            // one detached, hidden-background server child (logs beside the
            // launcher's). The request is then HELD until the fresh child
            // answers action=self, so the panel learns the real pid and sees
            // a failed launch instead of a silent no-op: a child that exits
            // early (lost the port race) costs exactly one retry on the next
            // free port; a merely slow-booting child is reported
            // start_unconfirmed and left alone — never double-spawned.
            ...
            if (!r.ok && r.code === 'start_failed') {
              // One retry on the next free port — the usual cause is having
              // lost the scan/bind race for the first pick.
          ...
          if (action === 'stop') {
            ...
            // answer there and never checked the method anyway. 8s per
            // attempt: a freshly launched target may still be initializing.

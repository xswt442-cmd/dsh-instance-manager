...
export default {
  ...
  apply(ctx) {
    ...
    const dshBin = () => {
      // Prefer the exact entry script this instance was started with, falling
      // back to the canonical profile location (fresh CI-like environments
      // lack the profile copy of @deepseek-ai/dsh).
      const arg = process.argv[1] || ''
      if (arg && /bin\.js$/.test(arg) && fs.existsSync(arg)) return arg
    ...
    const startInstance = () => {
      const node = findNode()
      const bin = dshBin()
      if (!node) return { ok: false, error: '未找到 node 可执行文件' }
    ...
    // Self-report across versions: current route first, then the pre-rename
    // path so <=0.4.x peers still answer discovery.
    const selfReportOf = async (port) =>
      await fetchJson(port, 'action=self', 2500) ||
      await fetchJson(port, 'action=self', 2500, 'GET', LEGACY_BASE)
    ...
    const listInstances = async () => {
      ...
      const results = await Promise.all(ports.map(async (port) => {
        const self = await selfReportOf(port)
    ...
    // Remove the alias once 0.4.x installs are gone.
    const apiRoute = {
      kind: 'exact',
    ...
    ctx.effect(() => [
      ...
      ws.register({ ...apiRoute, path: '/dsh-easy-port-manager/api' })

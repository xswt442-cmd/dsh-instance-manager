...
import { createGuard, resolveDshBin } from './shared.js'
...
export default {
  ...
  apply(ctx) {
    ...
    const startInstance = () => {
      ...
      if (!bin) return { ok: false, error: '未找到 dsh 启动器：' + path.join(dshHome(), 'profiles', 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js') }

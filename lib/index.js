    ...
    const apiRoute = {
      ...
      handler: async (req, res) => {
        try {
          ...
          // Dual-mode gate: a request is remote (and therefore must clear the
          // fleet bearer) when its real TCP peer is off-loopback OR its Host is
          // off-loopback. Judging by the socket peer closes the forged-loopback
          // Host bypass, where a remote attacker set Host: 127.0.0.1 and slipped
          // past both gates without a token. Unconfigured token = fail closed.
          if (requestNeedsBearer(req)) {

...
- **面板此前会吞掉操作失败的原因。** `refresh()` 在每次列表读取成功后都调用 `setError(null)`，而 `startNew()` 是先设错误再调 `refresh()`，因此「端口已被占用」「启动失败」这类提示在被读到之前就被清掉了 —— 表现就是点击后毫无反应。改为只有列表读取本身失败才写入该状态；操作错误保留到下一次操作。
- 因此 0.9.10 的端口占位检查虽然服务端正确返回 `port_in_use`，面板上却看不到任何提示。现在会显示。
